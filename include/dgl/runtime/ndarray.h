...
class NDArray {
 public:
  ...
  inline void CopyTo(const NDArray& other, const DGLStreamHandle& stream=nullptr) const;
  /*!
   * \brief Copy the data to another context.
   * \param ctx The target context.
   * \return The array under another context.
   */
  inline NDArray CopyTo(const DLContext& ctx, const DGLStreamHandle& stream = nullptr) const;
  /*!
...
inline void NDArray::CopyTo(const NDArray& other, const DGLStreamHandle& stream) const {
  CHECK(data_ != nullptr);
  CHECK(other.data_ != nullptr);
  CopyFromTo(&(data_->dl_tensor), &(other.data_->dl_tensor), stream);
}

inline NDArray NDArray::CopyTo(const DLContext& ctx, const DGLStreamHandle& stream) const {
  CHECK(data_ != nullptr);
