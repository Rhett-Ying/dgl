...
# attach node data to graph
print("start to attach node data to graph")
paper_feat_path = os.path.join(input_dir, "processed/paper/node_feat.npy")
g.nodes["paper"].data["feat"] = torch.from_numpy(np.load(paper_feat_path))
print(g.nodes["paper"].data["feat"].shape)

paper_label_path = os.path.join(input_dir, "processed/paper/node_label.npy")
g.nodes["paper"].data["label"] = torch.from_numpy(np.load(paper_label_path))
print(g.nodes["paper"].data["label"].shape)

paper_year_path = os.path.join(input_dir, "processed/paper/node_year.npy")
g.nodes["paper"].data["year"] = torch.from_numpy(np.load(paper_year_path))
print(g.nodes["paper"].data["year"].shape)

g.nodes["paper"].data["train_mask"] = torch.from_numpy(np.load(train_mask_path))
g.nodes["paper"].data["val_mask"] = torch.from_numpy(np.load(val_mask_path))
g.nodes["paper"].data["test_mask"] = torch.from_numpy(np.load(test_mask_path))


print("start to partition graph...")
graph_name = "ogb-mag240m"
output_dir = os.path.join(base_dir, "datasets/mag240m_kddcup2021/metisAPI_parted_data")
num_parts = 4
dgl.distributed.partition_graph(
    g, graph_name, num_parts, output_dir, balance_edges=True)
